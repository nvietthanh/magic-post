        ...
        <div>
            ...
            <el-form ref="form" :model="formDataChange" :rules="rules" label-position="top">
                <div class="flex gap-[32px]">
                    <div>
                        <div class="flex justify-between">
                            <span class="font-bold">Trạng thái đơn hàng:</span>
                        </div>
...
import { PENDING_APPROVAL, TRANSIT_TO_CONCENTRATE } from '@/Store/Consts/oderStatus'
...
export default {
    ...
    data: function () {
        return {
            ...
            transitConcentrateStatus: TRANSIT_TO_CONCENTRATE,
            isShowForm: false,
            loadingForm: false,
            formData: {},
            fields: [
    ...
    methods: {
        async open(data) {
            ...
            if (this.formData.status) {
                for (let item of this.formData.status) {
                    if (item.type == this.pedingStatus) {
                        this.formDataChange.statusOrder = item.status == 1 ? true : false
                    } else if (item.type == this.transitConcentrateStatus && item.status == 1) {
        ...
        submit() {
            ...
            axios.get(route('admin.api.transaction-order-send.peding-status', params))
                .then(({ data }) => {
