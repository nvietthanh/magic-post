<template>
    ...
    >
        ...
        <div>
            ...
            <div>
                <div class="flex justify-between">
                    <span class="font-bold">Trạng thái tiếp nhận:</span>
                </div>
...
import { TRANSIT_TO_CONCENTRATE, TRANSIT_TO_CONCENTRATE_DESTINATION } from '@/Store/Consts/oderStatus'
...
export default {
    ...
    data: function () {
        return {
            concentrateStatus: TRANSIT_TO_CONCENTRATE,
    ...
    methods: {
        async open(data) {
            ...
            if (this.formData.status) {
                ...
                for (let item of this.formData.status) {
                    if (item.type == this.concentrateStatus) {
                        this.statusOrder = item.status == 1 ? true : false
                    } else if (item.type == this.concentrateDistinationStatus) {
        ...
        changeStatus() {
            ...
            axios.get(route('admin.api.concentrate-order-receive.change-receive-status', params))
                .then(({ data }) => {
