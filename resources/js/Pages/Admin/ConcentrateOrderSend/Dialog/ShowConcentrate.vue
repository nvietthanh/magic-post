            ...
            <div>
                <el-form ref="form" :model="formDataChange" :rules="rules" label-position="top">
                    <div class="flex gap-[32px]">
                        <el-form-item label="Gửi đơn hàng đến điểm tập kết đích:" class="w-[300px]" prop="statusOrder"
                            :error="getError('statusOrder')" :inline-message="hasError('statusOrder')">
                            <el-switch size="large" v-model="formDataChange.statusOrder" :disabled="disableStatus"/>
                        </el-form-item>
                        <el-form-item v-if="formDataChange.statusOrder"
                            label="Chọn điểm tập kết đích:" class="w-[300px]" prop="concentrate_point_id"
                            ...
                            <el-select v-model="formDataChange.concentrate_point_id" 
                                class="w-full" placeholder="Chọn điểm tập kết"
...
import { TRANSIT_TO_CONCENTRATE_DESTINATION_SEND } from '@/Store/Consts/oderStatus'
...
export default {
    ...
    methods: {
        async open(data) {
            ...
            this.formDataChange.concentrate_point_id = ''
            this.formData = { ...data }
            for (let item of this.formData.status) {
                if (item.type == TRANSIT_TO_CONCENTRATE_DESTINATION_SEND) {
            ...
            axios.get(route('admin.api.concentrate-order-send.get-concentrate', this.formData.delivery_district_id))
                .then(({ data }) => {
        ...
        changeStatus() {
            ...
            axios.get(route('admin.api.concentrate-order-send.change-concentrate-send', params))
                .then(({ data }) => {
