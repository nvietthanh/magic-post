<template>
    ...
    >
        ...
        <div>
            ...
            <div>
                <div class="flex justify-between">
                    <span class="font-bold">Chọn điểm tập kết đích:</span>
                </div>
...
import { TRANSIT_TO_CONCENTRATE_DESTINATION } from '@/Store/Consts/oderStatus'
...
export default {
    ...
    data: function () {
        return {
            concentrateDistinationStatus: TRANSIT_TO_CONCENTRATE_DESTINATION,
            isShowForm: false,
            loadingForm: false,
            formData: {},
            fields: [
    ...
    methods: {
        async open(data) {
            ...
            if (this.formData.status) {
                for (let item of this.formData.status) {
                    if (item.type == this.concentrateDistinationStatus) {
                        this.formDataChange.concentrate_point_id = item.concentrate_point_id
                        if (item.status == 1) {
        ...
        changeStatus() {
            ...
            axios.get(route('admin.api.concentrate-order-send.change-send-status', params))
                .then(({ data }) => {
