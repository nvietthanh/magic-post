...
class TransactionOrderReceiveResource extends JsonResource
{
    /**
    ...
    public function getOrderStatus($orderStatus)
    {
        ...
        foreach ($orderStatus as $status) {
            if ($status->type == OrderStatusEnum::TRANSIT_TO_TRANSACTION_DESTINATION_SEND && 
                $status->receive_point_id == $currentUser->adminProfile->transaction_point_id)
            {
                $statusNumber = OrderStatusEnum::TRANSIT_TO_TRANSACTION_DESTINATION_SEND;
                break;
            } elseif ($status->type == OrderStatusEnum::RETURN_TO_TRANSACTION && 
                $status->receive_point_id == $currentUser->adminProfile->transaction_point_id)
            {
                $statusNumber = OrderStatusEnum::RETURN_TO_TRANSACTION;
                break;
            }
        }
